...
## <중요> 토글 후 새로고침(F5)해주셔야 적용된 결과가 보입니다.
...
- **설치 후 내려받은 폴더는 지워도 됩니다** — 실행에 필요한 파일이 모두 복사됩니다
...
`Install.bat` 은 실행에 필요한 파일을 모두 아래로 복사한 뒤, 우클릭 메뉴가 **그 사본**을 실행하도록 등록합니다.
...
그래서 **설치가 끝나면 내려받아 압축을 푼 폴더는 지워도 됩니다.**
제거 도구까지 함께 복사되므로 나중에 지울 때도 곤란하지 않습니다.

## 제거

**`Uninstall.bat` 을 더블클릭하십시오.** 내려받은 폴더를 이미 지웠다면
`%LOCALAPPDATA%\FileExtensionToggle\Uninstall.bat` 을 실행하면 됩니다.

메뉴 항목과 설치된 파일을 모두 지웁니다. 확장자 설정 자체는 그대로 둡니다.
...
`Install.bat` 을 다시 실행하십시오 (설치본이 갱신됩니다).

## 참고 / 제약

- `src/` 나 `icons/` 를 고쳐도 **`Install.bat` 을 다시 실행해야 반영됩니다.**
  메뉴는 내려받은 폴더가 아니라 설치본을 실행하기 때문입니다.

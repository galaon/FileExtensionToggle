...
| `icon-on.ico` / `icon-off.ico` | 상태별 아이콘 (파랑 / 회색) |
| `icon-source.svg` | 아이콘 원본 |
...
powershell -ExecutionPolicy Bypass -File ".\Install.ps1" -MenuText "확장자 보기"
```
...
다른 아이콘을 쓰려면 같은 이름의 `.ico` 두 개로 바꿔 넣고 `Install.bat` 을 다시 실행하십시오.

## 참고 / 제약

- **폴더를 옮기면 메뉴가 깨집니다.** 스크립트와 아이콘 경로가 절대경로로 기록되기 때문입니다.
